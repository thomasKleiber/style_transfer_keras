...
class styleMasked():
    ...
    ################################################################################
    def get_style_trees(self, K):
        self.ST = {}
        for layer in self.style_layers:
            sti = styleTree(layer)
            sti.get_FMs(self.pimi)
            sti.get_km(K)
            sti.get_masks()
            self.ST[layer] = sti
    ...
    ################################################################################
    def get_gramms_target(self, mask=None):
        ...
        for layer_name in self.style_layers:
            ...
            self.append_pred(pred, session, mask)
            x = input_img
            for i in range(len(s.pyrdowns)):
    ...
    ################################################################################
    def append_pred(self, p, session, mask):
        GI=list()
        fms = np.squeeze(p[0,:,:,:])
        masks = np.squeeze(mask[0])
        for i in range(1, p.shape[0]):
            fms=np.concatenate((fms, np.squeeze(p[i,:,:,:])), axis=0)
            masks=np.concatenate((masks, np.squeeze(mask[i])), axis=0)

        print('%s / %s' % (str(fms.shape), str(masks.shape)))
        G = gram_matrix(fms, masks)
        GI.append(G.eval(session=session))
        self.grams.append(np.array(GI))

    ################################################################################
    def get_gramms_target_from_vector(self, vec):
        for layer in self.style_layers:
            self.grams.append(self.ST[layer].vec_to_gramm(vec))
            for i in range(len(s.pyrdowns)):
                if s.pyrdowns[i] > 0:
                    self.grams.append(self.ST[layer].vec_to_gramm(vec))
    ...
    ################################################################################
    def gram_loss(self, j, G, chw):
        """ i index in self.grams, j image index """
        num = K.sum(K.square(np.squeeze(self.grams[0][j])-G))
    ...
    def iterate(self):
        ...
        for L in range(len(self.style_layers)):
            ...
            style_loss = style_loss \
                + self.gram_loss(self.pc*L, G, c*h*w)
            ...
            for i in range(len(s.pyrdowns)):
                if s.pyrdowns[i] > 0:
                    ...
                    style_lossu[i] = style_lossu[i] \
                        + self.gram_loss(self.pc*L + self.po[i], G, c*h*w)
        ...
        for i in range(self.ITS):
            ...
            if (i % self.log_granularity) == 0:
                ...
                if (lp < self.learning_rates[lr_idx][1]) \
                        and (np.isnan(lpd) or lpd < self.learning_rates[lr_idx][1]) :
                    lr_idx = lr_idx+1
